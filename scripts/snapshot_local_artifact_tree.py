...
def collect_entries(
    ...
    def add(path: Path) -> None:
        ...
        entries.append(
            {
                "schema_version": "1.1-artifact-tree-entry",
...
def render_tree(
    ...
    lines = [
        "# schema_version: 1.1-artifact-tree-snapshot",
        f"# producer_machine_id: {machine_id}",
...
def main() -> None:
    ...
    entries = collect_entries(repo_root, scan_roots)
    output_root = resolve_inside_repo(repo_root, args.output_root)
    metadata_path = output_root / f"{args.machine_id}_latest.meta.json"
    ...
    atomic_write_text(
        tree_path,
        render_tree(entries, args.machine_id, captured_at, scan_root_strings),
    )
    atomic_write_text(
        metadata_path,
        ...
        json.dumps(
            {
                ...
                "counts": counts,
                "file_bytes": total_bytes,
                "metadata": repo_relative(repo_root, metadata_path),
