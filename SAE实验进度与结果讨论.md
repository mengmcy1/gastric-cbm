...
- bootstrap worker/coordinator已按冻结患者有放回语义实现：同一患者的重复实例保留独立
...
- provenance blocker已在正式运行前关闭：runner启动时独占冻结Git commit、protocol bundle SHA与
  22个显式正式执行文件及直接依赖的逐文件SHA，续跑和finalizer再次逐项复验；`run_manifest`缺任一类血缘即
...
### 医学生补充材料：维度保真与逐图多Feature图谱（2026-08-26）

已交付主包不再覆盖，另生成独立补充Word：

```text
12_SAE维度扩宽与重建性能说明_医学生版.docx
```

该文档直接读取S2c正式config，说明C-long每个局部位置的1280维表示进入10240维SAE字典，
即8倍过完备字典；学生分类器本身未扩宽。文档列出原学生与`K=64/128/256/512/1024`五档
严格重建的患者AUC、Sensitivity、Specificity、Accuracy、F1、一致率和cosine。`K=1024`下
患者AUC由0.91014变为0.90960（-0.00054），Sensitivity保持0.89552，Specificity由0.73057
变为0.73575，Accuracy由0.77308变为0.77692；但冻结阈值患者一致率为94.23%，未达到预注册
95%门槛。因此总体性能变化很小，但严格重建型SAE仍未成为分类产品；当前技术解释继续采用
残差保留干预，不以SAE重建概率替代原学生预测。

新增病例中心全量图谱：

```text
结果/SAE/RP_SAE完整阶段成果_医学生提交版_v1_20260826/10_逐图多Feature图谱_v2/
```

该目录覆盖RP-D冻结病例中的1770张唯一train图像。每图从149个正式展示Anchor中按
`本图峰值 / 该Anchor冻结train正激活Q99`选Top-6，展示原图、Top-6多颜色叠加、六个单独
overlay、达到各自`0.5×Q99`的响应重合数和技术摘要；同时保存1770行Top-6汇总及
...
### 当前安排：医学审核与功能解释衔接（2026-09-08用户确认）

本次根据医学生反馈及`文献/SAE后续改进/`中的两篇论文调整后续安排。这里只记录已确认的
研究方向，不代表新增实验已经完成，也不新增或修改现有预注册数值门槛。以下安排为当前
下一步；本节后面的历史清单保留作追溯，不再直接作为即时启动顺序。

**已核对的事实与沟通修正**：

- 当前S2c/RP-SAE输入为C-long `features[8]`的`7×7×1280`局部特征，49个位置共享字典，
  位于注意力汇聚之前。早期pooled SAE才使用汇聚后的向量；对话中的“目前在池化后训练”
  不适用于当前版本，不应据此重复启动一次“改为池化前SAE”的实验。
- Mammo-SAE对局部特征训练SAE，结合概念标注、空间响应和特征组干预进行分析；当前项目
  已具备局部SAE基础，但癌/非癌标签及病灶框不能替代发红、凹陷等具体概念的人工标注。
- H&E基因表达解释论文在注意力汇聚后的表示上训练SAE，再将预测相关性传播回输入图像。
  它支持区分激活与预测贡献，不意味着“只有池化前SAE才能定位”，相关性归因也不等同于
  残差保留删除实验。

**近期任务与预期产物**：

1. 先复用现有149个Anchor、RP-C2干预结果及Decision-aware展示，整理小批医学审核样本。
   选例沿用已冻结的排名或rank-gap规则，不人工只挑病灶对齐的漂亮案例；暂不全量生成新图，
   不更换C-long或SAE、不重新训练。具体审核数量和执行细节在制作前明确。
2. 每个候选围绕三个问题组织材料：“对应什么视觉模式、模式在哪里、对当前预测有何影响”。
   包含跨患者高响应图与局部区域、低响应或反例、原始SAE空间响应和病灶框，以及重新计算
   注意力后的删除效应。注意力加权响应仅作辅助；固定注意力的局部贡献图不冒充完整干预图。
3. 医学命名阶段先看图及反例，再揭示癌/非癌统计和功能结果，避免先知道“强促癌”后反向
   命名。反光、器械、背景及无法命名的候选均保留。预期产物为候选名称、对应区域、跨患者
   一致性、反例和不确定性记录；未命名前继续使用Feature/Anchor ID。
4. 汇总表将“癌/非癌中的出现与响应强度”和“删除后影响方向与大小”分栏。激活率明确统计
   层级与定义，不等同于医学概念出现率；当前患者presence高度饱和，不能仅靠是否激活筛选。
   共享不等于中性、癌富集不等于促癌，平均效应接近零也须检查病例间正负抵消。沿用
   `delta_margin=ablated-original`：负值表示删除后癌分数下降，正值表示删除后癌分数上升。
5. 已有RP-A独立确认与val复现继续作为待完成任务，遵循原有前置条件和seed职责；不能因为
   医生给出了名称就跳过技术确认，也不能将42/43/44开发结果写成202/503正式复现。

**决策标准与后续分支**：

- 小批审核首先判断医生能否跨患者识别同一模式，并在未参与命名的病例上再次识别；不以
  热图更漂亮作为成功标准。只有病灶框时，只能评价病灶对齐，不能据此确认具体医学概念。
- 若模式稳定、只是激活排名与功能排名不同，继续完善当前展示和审核，不因此判定SAE失败。
- 若模式可辨但7×7响应不足以定位，再另立小规模概念条件输入归因研究，借鉴H&E论文；
  上采样或归因热图不能未经验证就当作精确病灶边界。
- 若跨患者始终无法形成一致语义，再考虑更早特征层、字典约束或结构修改，每次只检验一个
  明确假设，不直接扩大超参数矩阵。
- 候选研究点是把同一Feature的共享性、空间模式和预测作用连接到可复核的医学证据；其
  有效性及相对现有工作的增益尚未验证。若审核或独立复现失败，应保留技术候选/失败结论，
  不进入正式医学概念瓶颈训练。

本次只更新后续安排，未新增模型、图谱或实验结果。S2c正式失败、RP-A尚未完成确认、
RP-B/C/D为train-only探索性技术证据，以及internal test/external使用边界均保持原记录。

### SAE提取改进：RA-SAE后续规划（2026-09-08用户同意记录，未正式预注册）

本节承接医学审核与功能解释安排，研究重点转向“提取出的Feature是否稳定、能否形成一致的
视觉含义”。当前仅确定候选方向和对照原则，未实现、未训练RA-SAE，也未确认其优于现有方法。
医学生继续查看图片并自由描述，暂不要求填写表格；其反馈用于形成候选假设，不作为医学真值。

**研究问题与依据**：

- 拟检验：固定C-long后，让SAE字典方向受到真实训练特征约束，能否提高跨初始化稳定性及
  跨患者语义一致性，同时保留重构质量和可验证的预测作用。
- 借鉴H&E基因表达解释论文使用的Relaxed Archetypal SAE（RA-SAE）：以训练特征的代表点
  组合约束decoder方向，并允许有限偏离。进一步核对其原始论文及官方实现：
  `https://arxiv.org/abs/2502.12892`。H&E原文使用汇聚后表示；应用到本项目局部特征属于适配，
  不能称为原论文完整复现。论文结果不保证胃镜Feature必然医学化或与原注意力一致。
- Mammo-SAE主要借鉴概念区域核对和特征干预评价；本项目已使用局部SAE，不重复以“改成
  池化前训练”为新实验。H&E的相关性反传作为独立解释方向，不与字典改进同时引入。
- 已有patch损失采用`0.5 + 0.5*49*a_p`位置权重，已经重视高注意力区域。因此暂不优先增加
  注意力采样或强制所有Feature对齐原注意力；更像原注意力不单独作为提取成功标准。

**推进顺序与预期产物**：

1. 先核对RA-SAE原论文与代码的decoder约束、松弛项、中心化/标准化及推理方式，确认其如何
   适配现有`7×7×1280`特征、单位范数约定和残差保留干预。产物为最小实现方案与明确差异说明，
   不先创建新的训练或实验管理框架。
2. 冻结一个小规模双臂比较：现有自由字典作为对照，数据约束字典作为候选；两臂使用相同
   C-long、train/val患者划分、特征层、字典宽度、稀疏预算、训练预算和损失口径。优先只改变
   字典参数化；若归一化或其他改动不可避免，应明确记录并匹配对照，不能将组合收益归因于
   单一约束。历史S2c结果保留作参照，不替代预算不同的新实验对照。
3. 代表点及预处理统计只从train构建；val用于开发评价和事先规定的选择。先完成必要的CPU
   数学检查及小样本验证，再由用户启动正式比较；不同时扫描宽度、K、特征层和多个松弛强度。
   小样本成功只说明实现可运行，不代表科学假设成立。
4. 初步比较有依据后，才按事先确定的独立初始化重复及病例规则评价稳定性与医学语义。
   候选图片采用同等审核数量、统一展示规则及跨患者高低响应/反例；命名后用未参与命名的
   病例复核。医学命名、稳定性和功能作用分别报告，不合成未经验证的总分。

**评价和决策边界**：

- 主要关注跨次训练能否找到对应Feature、跨患者是否出现一致模式，以及是否有可重复的
  残差保留干预效应；重构及分类保真同时评价，不能靠牺牲保真获得看似简单的概念。
- 原注意力对齐与病灶框对齐作辅助诊断，不等同于医学概念正确；黑框等模式即使被稳定提取，
  也不自动代表改进成功。预测贡献大小与临床合理性分别判断。
- 若仅热图更集中、重构更好，或只是稳定提取伪特征，而语义/功能证据未改善，应报告对应
  有限收益，不宣布总体成功；若无改善，保留失败，不立即追加大范围参数搜索。
- 具体参数、重复次数、指标定义、容许保真损失及推进门槛在实现方案核对后、读取新比较结果
  前确定。本规划不改变原S2c失败结论、RP-A待确认状态或202/503/911的既有seed职责，不借用
  已锁定确认资源调新方法；internal test/external不用于本轮开发。
...
#### 7. 内部时间确认队列的事后Feature分析验证

本轮固定使用原100轮RA-SAE、字典宽度2560和`K=256`，不使用扩大抽样池或
标准化候选。队列直接复用已冻结的内部时间测试清单和Keep预处理，共
112张/78人，其中癌52张/32人、非癌60张/46人。原113张中的1张确认重复图
未重新纳入。

该队列与SAE train/val的患者ID、Keep图SHA和几何ID重叠均为0，没有参与
RA-SAE训练、排序定义、干预设计或医学选例；但它此前已用于C-long评价，
因此定位为“未参与RA-SAE开发的内部时间确认队列”，不是整个项目从未查看
的全新盲测集。使用原正式批量32时，C-long逐图概率、attention和患者聚合的
最大复现误差为`1.11×10⁻¹⁶`。

三项冻结评价结果为：

- 纯重构patch/pooled `1−FVU`为52.29%/46.35%；患者概率绝对变化均值为
  0.09318，预测一致率88.46%，患者AUC由0.83152降至0.81726。与完整
  val497张/260人相比，patch数值保真略高，pooled与预测保真较低，属于
  **泛化表现不均衡**，不是方法改变后的“改善不一致”。
- 梯度预测效应与实际删除效应的全2560项绝对值Spearman为0.99969；带符号
  Spearman为0.99943，后者只作方向关系补充，不与既有val98绝对效应排序
  指标混用。Top-6患者等权重合率为95.94%，与既有val98张/64人的96.09%
  描述值接近。按“逐图比例→患者内平均→患者等权”汇总，非近零效应的方向
  一致率为99.8023%。
- Top-20相对`k0`将患者概率误差减少33.19%，恢复25人并新增2人偏离，
  净减少23例翻转；患者预测一致率为80.77%，仍有15/78人与原预测不一致。
  患者概率绝对误差P95/最大值为0.50302/0.59982，因此仍不属于高保真复原。

阶段结论为：

> 固定RA-SAE的功能排序在内部时间确认队列上保持较高的删除排序一致性；
> Top-k保留仍具有部分预测恢复作用，但纯重构和少量Feature均未达到高保真复原。
> 该队列此前已用于C-long评价，本次结果不构成医学语义或临床解释有效性的验证。

上述队列间差异均为描述性对照，未做统计显著性判定，不使用测试结果回调排序、
删除、Top-k、阈值或模型。外部集未纳入本阶段。医学阅片、Concept Family和CBM
仍保持暂停。

结果和代码入口：

- [内部时间确认队列结果](结果/SAE/RA_SAE_Pilot_20260908/internal_test_posthoc_validation/结果说明.md)；
- [`evaluate_clong_rasae_internal_test.py`](程序/SAE/正式代码/evaluate_clong_rasae_internal_test.py)。
