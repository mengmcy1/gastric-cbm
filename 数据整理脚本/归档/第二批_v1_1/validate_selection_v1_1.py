"""独立验收第二批 P2 v1.1 dHash+SSIM 去重与限图结果。"""
...
def independent_ssim(left, right):
    c1 = (0.01 * 255) ** 2
    c2 = (0.03 * 255) ** 2
...
def expected_components(hashes, paths):
    ...
    for left in range(len(hashes)):
        for right in range(left + 1, len(hashes)):
            if hamming_hex(hashes[left], hashes[right]) <= DHASH_THRESHOLD:
                ...
                score = independent_ssim(feature_cache[left], feature_cache[right])
...
def validate_patient(group):
    ...
    for component in expected_components(hashes, path_by_local):
    ...
    for duplicate_group, duplicate_rows in group.groupby('duplicate_group', sort=False):
        ...
        if len(duplicate_rows) > 1:
            minimum_ssim = duplicate_rows['duplicate_group_min_confirmed_ssim'].min()
            if pd.isna(minimum_ssim) or minimum_ssim < SSIM_THRESHOLD:
                patient_flags.append('invalid_duplicate_group_ssim')
