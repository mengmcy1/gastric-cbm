...
def parse_args():
    parser = argparse.ArgumentParser(description='第二批数据患者内去重与限图 v1')
...
def duplicate_components(hashes):
    ...
    return sorted(components.values(), key=lambda indexes: min(indexes))
...
def choose_patient_images(group):
    ...
    components = duplicate_components(hashes)
    representatives = []
    duplicate_meta = {}
    ...
    for number, component in enumerate(components, start=1):
        ...
        for local_index in component:
            duplicate_meta[local_index] = (group_id, len(component), representative)
    ...
    for local_index, dataframe_index in enumerate(indexes):
        group_id, group_size, representative = duplicate_meta[local_index]
