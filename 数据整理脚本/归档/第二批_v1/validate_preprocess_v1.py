...
def parse_args():
    parser = argparse.ArgumentParser(description='第二批预处理结果验收 v1')
...
def output_paths(mode):
    ...
    data_dir = os.path.join(DATA_ROOT, f'第二批裁剪后_v1{suffix}')
...
def validate_row(row, data_dir):
    ...
    if float(row.roi_area_ratio) < MIN_ROI_AREA_RATIO:
