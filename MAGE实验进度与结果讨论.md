...
由正在并行维护的`SAE实验进度与结果讨论.md`单独记录；CADe由
`CADe实验进度与结果讨论.md`独立记录。三条线互不覆盖，MAGE逐轮日志只写入本文。
...
| internal test/external | 外部分类、外部bbox空间评价与新内部时间测试均完成 | 外部539张标框癌图上C-long PGA=0.8108，显著高于A-long 0.3544和M0-F CAM 0.6437；外部v2患者AUC=0.7639。新内部集C-long患者AUC=0.8315、PGA=0.9231 |
| 外部癌图bbox | 自动与全量肉眼验收均通过 | 542张中539张有唯一合法癌框；3张病灶不清按临床反馈排除，不损失患者；539张三联图及30页总览于2026-08-25确认无问题 |
...
16. 医学生返回的`数据/标注-外部测试集胃镜多中心.xml`已于2026-08-24完成结构化
    自动验收：XML 542张与交接清单逐张一致，539张有且仅有一个合法癌框，
    无越界、无非正宽高、无重名；3张因病灶不清未标框，按临床反馈只从病灶
    空间评价中排除，不删除原图也不改写既有外部分类队列。排除后仍为121位癌患者。
    v2可评价/排除清单、自动验收汇总与539张“原Keep图+医生框+框内局部”三联图位于
    `03_外部癌图标注整合_v2_20260824`；下一步由用户全量肉眼复核，通过后才进入外部
    AiB/normalized AiB/PGA描述性评价。为便于复核，539例另按每页18例整理为30张分页总览；
    用户已于2026-08-25完成全量肉眼检查并确认全部通过，外部空间评价清单正式可用。
...
## 外部多中心bbox空间关注描述性评价结果（2026-08-25）

医学生完成542张外部癌图标框后，3张病灶不清图按临床反馈排除；其余539张、121位
癌患者完成自动检查、全量三联图肉眼验收和正式空间评价。A-long/C-long复用2026-08-19
正式外部投影保存的内置7×7注意力，M0-F使用同一冻结checkpoint补算后验癌证据CAM。
本轮不重扫阈值、不调整checkpoint，也不回灌训练。

| 模型 | 空间图性质 | 可评价图/患者 | 平均AiB | 平均normalized AiB | PGA | 峰值入框 |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| M0-F | 后验CAM | 536/121 | 0.5006 | 0.3795 | 0.6437 | 345/536 |
| A-long | 内置注意力 | 539/121 | 0.3093 | 0.0869 | 0.3544 | 191/539 |
| **C-long** | **内置注意力** | **539/121** | **0.5211** | **0.4042** | **0.8108** | **437/539** |

M0-F有3张图在整张7×7特征图上均无ReLU后的正向癌证据，因此CAM数学上不可定义；
这3张被显式记录为缺失，没有用均匀热图或其他默认图填补。三位患者均有其他可评价图，
所以M0-F仍覆盖121位患者。M0-F CAM前向癌概率与8月19日保存概率最大绝对差为0.00385，
val AUC和冻结混淆矩阵复现通过；空间结果仍按描述性诊断解释。

患者配对bootstrap显示，C-long相对A-long的AiB、normalized AiB和PGA分别提高
`+0.2037`、`+0.2977`和`+0.4541`，三项95%CI均未跨0。相对M0-F，C-long的
AiB差值为`+0.0252`（95%CI `[-0.0035, 0.0566]`），normalized AiB差值为
`+0.0310`（95%CI `[-0.0081, 0.0731]`），两项均跨0；但PGA提高`+0.1784`
（95%CI `[0.1242, 0.2338]`），未跨0。

这说明最稳健的外部结论不是“C-long把更多总注意力质量搬进病灶框”，而是
**C-long更稳定地把最强关注峰值对准病灶**。这一模式与新内部时间测试一致：M0-F本身
已经包含不少病灶证据，MAGE的主要价值是减少最强关注点错位，而不是让模型从完全不看
病灶变成开始看病灶。
...
| --- | ---: | ---: | ---: |
| M0-F | 0.4531 | 0.7123 | 0.8966 |
| A-long | 0.1619 | 0.4178 | 0.6164 |
| **C-long** | **0.6518** | **0.9247** | **0.9658** |

排除3张病灶不清图后的分类v2队列为1938张/1329人，患者级结果为：

| 模型 | 患者AUC | Sensitivity | Specificity | Accuracy |
| --- | ---: | ---: | ---: | ---: |
| M0-F | 0.7412 | 0.7603 | 0.6300 | 0.6418 |
| A-long | 0.7378 | 0.8182 | 0.5397 | 0.5651 |
| **C-long** | **0.7639** | 0.8017 | 0.5704 | 0.5914 |

与原1941张结果相比，三模型患者AUC变化均不超过0.0007，Sensitivity和Specificity
完全不变，因此三张图的临床排除没有实质改写既有分类结论。

**结论边界**：该外部队列此前已参与旧工程决策，仍属于开发/验证性质，不是全新独立
确证集；M0-F后验CAM与A/C内置注意力也不是完全同构机制。上述结果支持MAGE关注校正
在多中心数据上具有外部一致性，但不单独构成最终临床泛化确证。

**产物**：`结果/MAGE/外部多中心空间关注描述性评价_20260825/`保存逐图空间指标、
v2分类指标、患者预测、bootstrap汇总、3张M0 CAM不可定义记录和57张诊断四联图。

## 医学生完整阶段成果提交包（2026-08-25）

在内部时间测试、外部多中心分类、外部539张癌图空间评价和既有人工QC均完成后，
已生成面向医学生的完整展示包：

`结果/MAGE/MAGE完整阶段成果_医学生提交版_v1_20260825/`

提交包包含：详细Word解读指南、核心结果与术语Excel、5张统计图、MAGE通俗流程图、
6张内部代表四联图、12张外部代表四联图、10张既有人工QC案例、57张外部完整诊断
四联图附录及可复算CSV。Word中文字体设为宋体，英文和数字设为Times New Roman。

展示口径固定为：C-long相对A-long的空间关注改善在内部和外部均稳定成立；相对M0-F
最稳健的外部优势是PGA，而不是全部注意力质量或分类AUC均已统计显著提高。提交包明确
保留小病灶、褶皱、反光、气泡和器械邻近干扰等失败边界，也明确外部多中心属于开发/
描述性验证而非完全独立确证。

同目录`09_数据来源与研究边界说明/MAGE临时产物清理建议.txt`列出了已被正式产物
替代的debug、冒烟、失败运行和旧阶段性展示包，预计手动清理后可释放约325 MB；正式
checkpoint、配置、预测、人工QC源图和一次性评价结果均列为必须保留。
