#!/usr/bin/env python3
"""EfficientNet-B0 M5：预测ROI融合分类部署实验——两类全部用冻结M1预测框ROI。

按M5预注册（2026-08-12）：癌图与非癌图全部使用冻结M1 Top-1预测框生成的ROI，不提供
真值框；M3c-B门控只控制框展示，不控制分类分支（gate_pass仅作分层）。冷启动训练独立
局部Encoder副本+局部分类头+融合分类头；M1全局分支全程冻结。

与M4的关键差异：
- ROI来源：癌图从真值框改为M1预测框（两类一致）；
- roi_label：以扩边后roi_crop_bbox与GT病灶覆盖率匹配（0=不含病灶/1=明确含/2=ignore），
  不再直接取图像标签；ignore行的局部CE被mask（cross_entropy ignore_index）；
- 审计：A/B/C沿用M4同名便于比较，新增A_crop（roi_crop_bbox几何）辅助审计；
- 诊断：患者级配对AUC_M5−AUC_M4 bootstrap（读M4 val患者预测）+ 单seed Δ比值，不作判据。

输入：冻结M5 ROI清单（`build_m5_roi_manifest.py`产物，含roi_label/gate_pass）。
正式参数见`实验进度与结果讨论.md`的M5预注册。
"""
...
DEFAULT_OUTPUT = PROJECT_ROOT / "结果/M5预测ROI融合_0804/正式验证集筛选"
...
# roi_label约定（与build_m5_roi_manifest.py一致）：0=不含病灶，1=明确含病灶，2=ignore。
IGNORE = 2
# M4正式成功均值（预注册后固定），启动器用整体存活率=mean(ΔM5)/此值。
M4_MEAN_DELTA = 0.0334
...
def parse_args():
    ...
    parser.add_argument("--self-test", action="store_true",
                        help="运行bootstrap聚合与ignore掩码损失回归自测后退出。")
...
def load_roi_manifest(path, image_root, debug, debug_units, seed):
    """校验并加载M5冻结ROI清单，仅保留train/val；debug时按split+标签抽样患者。
    ...
    返回:
        pd.DataFrame: train/val行，含roi_crop_bbox三套坐标、noncancer_stratum、
        roi_label(0/1/2)与gate_pass。
    校验: 只含train/val、无患者跨split/跨标签、roi_label取值合法、ROI裁剪框在界、
    图像存在。
    """
...
class M5Dataset(Dataset):
    """读取M5冻结ROI清单：完整图走全局分支(eval变换)，ROI裁图走局部分支。
    ...
    __getitem__返回:
        image[3,224,224]、roi[3,224,224]、label(long)、roi_label(long，0/1/2)、
...
class M5Model(nn.Module):
    """冻结M1全局分支 + 独立局部Encoder副本 + 局部分类头 + 融合分类头（同M4）。
...
def set_eval_mode(model):
    """将M5模型整体置于eval，并确保全局与局部features均保持eval（BN用running统计）。"""
    model.eval()
    model.global_features.eval()
    model.local_features.eval()


def set_m5_train_mode(model, stage):
...
def frozen_m1_snapshot(model):
    ...
    说明: M1定位头与M3c-B门控不在M5训练图中，不由本函数校验；它们由ROI清单产物链SHA
    保证来源一致。
    """
    state = {}
...
def compute_losses(outputs, labels, roi_labels, args):
    """M5总损失：融合CE + beta*局部CE（局部只在roi_label=0/1上算，ignore被mask）。

    参数:
        outputs: 模型输出（含fusion_logits[B,2]、local_logits[B,2]）。
        labels: 图像标签[B]；roi_labels: ROI标签[B]（0=不含病灶/1=含/2=ignore）。
        args: 含beta（默认0.5）。
    返回:
        dict: total/fusion/local三个标量损失。
    梯度路径: L_fusion经融合头更新融合头与未冻结局部Encoder（不经过局部分类头）；
    L_local更新局部分类头与未冻结局部Encoder，ignore行不参与。
    行为: 癌图预测框未拍到病灶时，局部头被监督为非癌ROI、融合头仍被监督为癌——
    融合头需学会"局部ROI无病灶证据时仍依赖全局特征完成分类"。
    """
    classification = nn.functional.cross_entropy(outputs["fusion_logits"], labels)
    # 局部CE只在roi_label=0/1的行上算；全为ignore时取0（cross_entropy的ignore_index
    # 在空有效集上会返回NaN，需显式处理）。
    valid = roi_labels.ne(IGNORE)
    if valid.any():
        local = nn.functional.cross_entropy(
            outputs["local_logits"][valid], roi_labels[valid]
        )
    else:
        local = outputs["local_logits"].sum() * 0.0
...
def patient_top2_mean(predictions, probability_column, group_column="patient_id"):
    ...
    返回:
        DataFrame: 列[group_column, label, patient_probability]，每单元一行。
    """
    def agg(group):
...
def evaluate(model, loader, device, args):
    ...
    参数:
        model: M5Model实例；loader: 该split的DataLoader；device: cuda/cpu；
...
def metadata_audit(frame):
    """四档元数据审计：A预测框几何 / A_crop裁图几何 / B q_region-only / C几何+q_region。

    参数:
        frame: 含train/val的冻结ROI清单（含split、roi_base_bbox、roi_crop_bbox、
        q_region、label）。
    返回:
        dict: 各档AUC；用train元数据拟合LogisticRegression，在val上评估，避免同集
        拟合评估的乐观偏差。标签退化时明确返回0.5，其他错误直接抛出不伪装。
    说明: A用roi_base_bbox（=两类预测框）与M4同名保持可比；A_crop用roi_crop_bbox，
    更贴近局部Encoder实际看到的裁图尺度。
    """
...
def compute_auxiliary_metrics(predictions, args, roi_config):
    """协议要求的辅助指标：局部头/hard-easy/gate分层/临床阈值/分层/一致率/新增对错患者。
    ...
    返回:
        dict: local_head_auc、local_auc_cancer_vs_{hard,easy}、local_acc_{hard,easy}、
              local_auc_cancer_roi_hit、patient_sens/spec_{sens90,default_0_5}、
              patient_threshold_sens90、stratified(来源/中心/分辨率/gate)、
              lesion_size(小中大病灶)、agreement_default_0_5与方向计数、
              new_correct/new_error_patients。
    """
    result = {}
    local = predictions.loc[predictions.split.eq("val")].copy()
    # 局部头整体AUC：以roi_label（含病灶=1、不含=0）为真值，ignore行不参与。
    known = local.loc[local.roi_label.ne(IGNORE)].copy()
    if known.roi_label.nunique() >= 2:
        result["local_head_auc"] = float(roc_auc_score(
            known.roi_label, known.local_probability
        ))
    else:
        result["local_head_auc"] = 0.5
    # 癌ROI(roi_label=1) vs hard/easy 非癌ROI 的局部AUC（局部分支对两类难度的区分力）。
    ...
    for name, stratum in [("cancer_vs_hard", "hard"), ("cancer_vs_easy", "easy")]:
        ...
        sub = pd.concat([
            cancer_roi[["roi_label", "local_probability"]],
            non[["roi_label", "local_probability"]],
        ])
    ...
    # 癌图内：预测框拍到病灶(1) vs 没拍到(0) 的局部AUC（局部头能否识别漏检）。
    cancer_known = local.loc[local.label.eq(1) & local.roi_label.ne(IGNORE)]
    if cancer_known.roi_label.nunique() >= 2:
        result["local_auc_cancer_roi_hit"] = float(roc_auc_score(
            cancer_known.roi_label, cancer_known.local_probability
        ))
    else:
        result["local_auc_cancer_roi_hit"] = 0.5
    # hard/easy准确率：局部头阈值0.5下对roi_label的判对比例（非癌恒0）。
    ...
    # 来源/中心/分辨率分层 + 门控分层：图像级与患者级融合AUC，报告每层图片/患者数。
    result["stratified"] = {}
    stratify_cols = [column for column in ["source", "center", "size_group"]
                     if column in local.columns]
    for group_col in stratify_cols:
    ...
    for gate_key, gate_value in [("gate:pass", 1), ("gate:fail", 0)]:
        group = local.loc[local.gate_pass.eq(gate_value)]
        if len(group) and group.label.nunique() >= 2:
            group_patients = patient_top2_mean(group, "fusion_probability")
            result["stratified"][gate_key] = {
                "n_images": int(len(group)),
                "n_patients": int(group_patients.patient_id.nunique()),
                "image_auc": float(roc_auc_score(
                    group.label, group.fusion_probability
...
def survival_diagnostics(patient_final, seed, m5_delta, iterations):
    """诊断：预测ROI vs 真值ROI的性能差（患者级配对bootstrap）+ 单seed Δ比值。

    读M4同seed的val患者预测与config；同一val患者集合上配对bootstrap计算
    AUC_M5 − AUC_M4 的95%CI，直接回答"从真值ROI换成预测ROI后性能损失是否显著"。
    另报告单seed Δ比值 = ΔM5/ΔM4。均不参与checkpoint选择；M4产物缺失或无法对齐时
    返回available=False。

    参数:
        patient_final: M5患者级表（patient_id/label/patient_probability）。
        seed: 随机种子。m5_delta: 该seed的Δ患者AUC（融合−全局）。
        iterations: bootstrap次数。
    返回: dict诊断结果。
    """
    ...
    m4_patients = pd.read_csv(
        m4_patient_csv, encoding="utf-8-sig", dtype={"patient_id": str}
    )
    ...
    merged = patient_final.merge(
        ...
        on="patient_id", suffixes=("_m5", "_m4"),
        validate="one_to_one",
    )
    if len(merged) != len(patient_final) or len(merged) != len(m4_patients):
        return {
            "available": False,
            "reason": "M4与M5的val患者集合不完全一致",
            "aligned_patients": int(len(merged)),
            "m5_patients": int(len(patient_final)),
            "m4_patients": int(len(m4_patients)),
        }
    if not np.array_equal(
        merged.label_m5.to_numpy(dtype=int),
        merged.label_m4.to_numpy(dtype=int),
    ):
        raise ValueError("M4与M5同名患者的标签不一致")
    merged["label"] = merged.label_m5.astype(int)
    ...
    for iteration in range(iterations):
        sample = rng.choice(patients, size=len(patients), replace=True)
        sample_df = merged.set_index("patient_id").loc[sample].reset_index()
        if sample_df.label.nunique() < 2:
            continue
        auc_m5 = float(roc_auc_score(
            sample_df.label, sample_df.patient_probability_m5
        ))
        auc_m4 = float(roc_auc_score(
            sample_df.label, sample_df.patient_probability_m4
        ))
        deltas.append(auc_m5 - auc_m4)
    deltas = np.asarray(deltas)
    return {
        "available": True,
        "aligned_patients": int(len(merged)),
        "m4_delta_patient_auc": m4_delta,
        "m5_delta_patient_auc": m5_delta,
        "delta_ratio_m5_over_m4": (m5_delta / m4_delta) if m4_delta > 0 else None,
        "auc_m5_minus_m4_ci95": [
...
def prepare_output(args):
    ...
    """
    name = args.run_name or f"m5_balanced_keep_efficientnet_b0_seed{args.seed}"
    if args.debug:
        name += "_debug"
...
def self_test():
    """回归自测：Dataset变换、bootstrap聚合与ignore掩码损失。"""
    dataset = M5Dataset(
        pd.DataFrame(), Path("."), False,
        eval_transform="eval", roi_train_transform="train", roi_eval_transform="roi_eval",
    )
    if dataset.eval_transform != "eval" or dataset.roi_transform != "roi_eval":
        raise AssertionError("M5Dataset未正确保存全局/局部变换")
    frame = pd.DataFrame({
    ...
    # ignore掩码损失：全ignore时局部损失必须为0；混合时只算有效行。
    class _Args:
        beta = 0.5
    args = _Args()
    local_logits = torch.tensor([[3.0, 1.0], [2.0, 2.0]], requires_grad=True)
    fusion_logits = torch.tensor([[0.5, 0.5], [0.5, 0.5]], requires_grad=True)
    labels = torch.tensor([1, 0])
    outputs = {"local_logits": local_logits, "fusion_logits": fusion_logits}
    losses = compute_losses(
        outputs, labels, torch.tensor([2, 2], dtype=torch.long), args
    )
    if float(losses["local"].detach()) != 0.0:
        raise AssertionError(
            f"全ignore的局部损失应为0，实际{float(losses['local'].detach())}"
        )
    losses = compute_losses(
        outputs, labels, torch.tensor([1, 2], dtype=torch.long), args
    )
    expected = nn.functional.cross_entropy(local_logits[:1], torch.tensor([1]))
    if abs(float(losses["local"].detach()) - float(expected.detach())) > 1e-6:
        raise AssertionError("ignore掩码损失与单行CE不一致")
    return True


def main():
    """编排M5训练：产物链校验、两阶段训练、选择、冻结校验、审计与保存。

    流程: 校验M5 ROI清单与M1 checkpoint（SHA/计数/debug）→ 构建平衡采样DataLoader →
    阶段A(只训两头)→阶段B(解冻features[7:])→按(患者AUC,图像AUC,-val融合损失)选产品 →
    冻结校验全局分支 → 输出m5_best.pth、预测、元数据审计、bootstrap、辅助指标与
    存活率诊断。无返回值；test/external恒False。
    """
    args = parse_args()
    if args.self_test:
        self_test()
        print("M5融合自测通过: Dataset变换 + bootstrap聚合 + ignore掩码损失")
        return
    if args.debug:
    ...
    model = M5Model(m1.backbone).to(device)
    ...
    # 基线：冻结全局分类（只用eval变换），M5融合提升以此为参照。
    ...
    # 冻结校验：M1全局分支参数/BN不变（M1定位头与M3c-B门控由ROI产物链SHA保证）。
    ...
    survival = survival_diagnostics(
        patient_final, args.seed, delta_auc, args.bootstrap
    )
    ...
    print(
        f"冻结全局患者AUC={config['m5_selection']['patient_global_auc']:.4f} -> "
        f"M5融合患者AUC={config['m5_selection']['patient_fusion_auc']:.4f} | "
        f"Δ={delta_auc:.4f}"
    )
    if survival.get("available"):
        print(f"存活率诊断: ΔM5/ΔM4={survival.get('delta_ratio_m5_over_m4')} "
              f"AUC_M5−AUC_M4 CI={survival.get('auc_m5_minus_m4_ci95')}")
    print(f"元数据审计: {audit}")
    print(f"输出目录: {output}")
