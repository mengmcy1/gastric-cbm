#!/usr/bin/env bash
# M5正式矩阵：冻结M1全局/定位基座，两类都用M1预测框ROI训练局部Encoder+局部头+融合头。
...
ENTRY="程序/模型训练/正式代码/efficientnet_m5_fusion.py"
OUTPUT_ROOT="$PROJECT_ROOT/结果/M5预测ROI融合_0804/正式验证集筛选"
ROI_ROOT="$PROJECT_ROOT/结果/M5预测ROI融合_0804/冻结ROI清单"
LOG_ROOT="$OUTPUT_ROOT/logs"
CUDA_DEVICE="${CUDA_DEVICE:-}"
# M4正式成功平均Δ（预注册后固定），用于整体存活率=mean(ΔM5)/此值。
...
usage() {
    cat <<EOF
用法:
  run_m5_fusion_matrix.sh <42|202|503|逗号分隔列表>

示例:
  nvidia-smi
  CUDA_DEVICE=<空闲GPU> bash 程序/模型训练/正式代码/run_m5_fusion_matrix.sh 42,202,503

前置:
  先运行 run_m5_roi_manifest_matrix.sh 42,202,503 生成三个seed的正式冻结ROI清单。

说明:
  - 幂等跳过只接受config含m5_selection的完整产物。
  - 任何seed失败，矩阵最终返回非零；不静默吞错。
  - 正式成功按预注册分级：强成功/正式成功/提示性收益/失败；矩阵结束只报告，不补救。
  - 整体存活率=mean(ΔM5)/${M4_MEAN_DELTA}（M4平均Δ），诊断指标不作判据。
EOF
}
...
report_summary() {
    ...
    for seed in 42 202 503; do
        local cfg="$OUTPUT_ROOT/m5_balanced_keep_efficientnet_b0_seed${seed}/config.json"
        if [[ ! -f "$cfg" ]]; then
...
value = c.get('m5_selection', {}).get('delta_patient_auc')
...
")"
    ...
    if [[ "$count" -gt 0 ]]; then
        ...
        echo "当前完成${count}/3种子；已完成seed平均Δ患者AUC=$mean"
        if [[ "$count" -ne 3 ]]; then
            echo "结论: 阶段性结果（未齐3/3，不执行M5正式成败判定）"
            return
        fi
        if [[ "$all_positive" == "1" ]] \
            && "$PYTHON" -c "exit(0 if $mean >= 0.02 else 1)"; then
            if [[ "$all_strong" == "1" ]]; then
                echo "结论: 强成功（3/3每个seed Δ>=0.02）"
            else
                echo "结论: 正式成功（3/3都提高，平均>=0.02）"
            fi
        elif [[ "$all_positive" == "1" ]] \
            && "$PYTHON" -c "exit(0 if 0.01 <= $mean < 0.02 else 1)"; then
            echo "结论: 提示性收益（3/3都提高，平均Δ∈[0.01,0.02)）"
        else
            echo "结论: 失败（存在seed下降，或平均Δ<0.01）"
        fi
        local survival
        survival="$("$PYTHON" -c "print($mean / ${M4_MEAN_DELTA})")"
        echo "整体存活率=mean(ΔM5)/${M4_MEAN_DELTA}=$survival（诊断指标，不作判据）"
    fi
}
...
main() {
    ...
    report_summary
    if [[ "$failed" -ne 0 ]]; then
        echo "存在失败seed，M5训练矩阵返回非零。" >&2
        exit 1
    fi
    echo "M5训练矩阵运行完成。日志目录: $LOG_ROOT"
}
