#!/usr/bin/env bash
# seed42 A1b：在8倍膨胀字典上比较四档逐样本Top-K稀疏度。
set -uo pipefail
...
OUTPUT="$PROJECT_ROOT/结果/SAE/EfficientNet全局局部_0804/SAE-A1b_TopK比较"
MODE="${1:-all}"
...
for branch in "${branches[@]}"; do
  ...
  for topk in "${topks[@]}"; do
    experiment="a1b_${branch}_efficientnet_b0_seed42_h10240_topk${topk}"
    ...
    if CUDA_VISIBLE_DEVICES="$CUDA_DEVICE" "$PYTHON" "$SCRIPT" \
        ...
        --activation-mode topk --top-k "$topk" --lambda-l1 0 \
