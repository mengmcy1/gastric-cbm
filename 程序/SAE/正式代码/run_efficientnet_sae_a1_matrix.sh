#!/usr/bin/env bash
# seed42 A1：全局/局部分别比较S1/S2与三档L1，复用A0冻结GAP特征。
set -uo pipefail
...
for branch in "${branches[@]}"; do
  ...
  for width in "${widths[@]}"; do
    stage="s1"; [[ "$width" == "2560" ]] && stage="s2"
    for index in "${!lambdas[@]}"; do
      lambda="${lambdas[$index]}"
      lname="${lambda_names[$index]}"
      experiment="a1_${branch}_efficientnet_b0_seed42_${stage}_h${width}_l1_${lname}"
      ...
      if CUDA_VISIBLE_DEVICES="$CUDA_DEVICE" "$PYTHON" "$SCRIPT" \
          --branch "$branch" --seed 42 --hidden-dim "$width" --lambda-l1 "$lambda" \
