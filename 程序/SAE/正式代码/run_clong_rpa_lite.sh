#!/usr/bin/env bash
# 独立运行train-only RP-A-lite B100；不调用正式coordinator/val/finalizer。
set -Eeuo pipefail
...
CUDA_DEVICES="${CUDA_DEVICES:?Lite启动前必须显式设置3张CUDA_DEVICES}"
IFS=',' read -r -a GPUS <<< "$CUDA_DEVICES"

if [[ "${#GPUS[@]}" -ne 3 ]]; then
  echo "RP-A-lite固定使用3个worker" >&2
  exit 1
fi
if [[ -e "$OUTPUT_ROOT" ]]; then
  echo "RP-A-lite输出根已存在，禁止覆盖: $OUTPUT_ROOT" >&2
  exit 1
fi
for gpu in "${GPUS[@]}"; do
...
echo "[$(date '+%F %T')] START replicate53 block equivalence" \
  | tee "$LOG_DIR/rpa_lite_runner.log"
CUDA_VISIBLE_DEVICES="${GPUS[0]}" "$PYTHON" -u "$CODE/clong_rpa_lite_worker.py" \
  --mode equivalence --worker-rank 0 --worker-count 1 --device cuda \
  --source-block 32 --target-block 128 \
  >"$LOG_DIR/rpa_lite_equivalence.log" 2>&1
PYTHONPATH="$CODE" "$PYTHON" - <<'PY' | tee "$LOG_DIR/rpa_lite_equivalence_check.log"
...
new53 = load_jsonl(Path("/home/mcy/gastric-cbm/结果/SAE/RP_A_Lite_Exploratory_20260825/equivalence/replicate53_32x128.jsonl"))[0]
difference = max_metric_difference(old53, new53)
print(f"replicate53_32x128_max_absolute_difference={difference}")
if difference != 0.0:
    raise SystemExit("32x128等价性未通过")
PY
echo "[$(date '+%F %T')] DONE replicate53 block equivalence" \
  | tee -a "$LOG_DIR/rpa_lite_runner.log"

pids=()
logs=()
for rank in 0 1 2; do
  log="$LOG_DIR/rpa_lite_worker${rank}.log"
  CUDA_VISIBLE_DEVICES="${GPUS[$rank]}" "$PYTHON" -u "$CODE/clong_rpa_lite_worker.py" \
    --mode missing --worker-rank "$rank" --worker-count 3 --device cuda \
    --source-block 32 --target-block 128 >"$log" 2>&1 &
  pids+=("$!")
  logs+=("$log")
  echo "Lite worker $rank -> GPU ${GPUS[$rank]}, log=$log" \
    | tee -a "$LOG_DIR/rpa_lite_runner.log"
done
...
while [[ "${#remaining[@]}" -gt 0 ]]; do
  ...
  if [[ "$status" -ne 0 ]]; then
    ...
    echo "Lite worker失败，保留现场" | tee -a "$LOG_DIR/rpa_lite_runner.log"
    exit "$status"
  fi
done

"$PYTHON" -u "$CODE/summarize_clong_rpa_lite.py" \
  2>&1 | tee "$LOG_DIR/rpa_lite_summary.log"
echo "[$(date '+%F %T')] RP-A-lite completed" \
  | tee -a "$LOG_DIR/rpa_lite_runner.log"
