#!/usr/bin/env bash
# seed42 S4终点诊断：全局/局部分别训练8倍膨胀字典，检验单纯扩维的保真上限。
set -uo pipefail
...
for branch in "${branches[@]}"; do
  ...
  for index in "${!lambdas[@]}"; do
    ...
    experiment="a1_${branch}_efficientnet_b0_seed42_s4_h10240_l1_${lname}"
    ...
    if CUDA_VISIBLE_DEVICES="$CUDA_DEVICE" "$PYTHON" "$SCRIPT" \
        --branch "$branch" --seed 42 --hidden-dim 10240 --lambda-l1 "$lambda" \
...
if ((${#failures[@]})); then
  printf 'SAE-A1-S4失败项: %s\n' "${failures[*]}"
  exit 1
fi
echo "SAE-A1-S4矩阵完成: $OUTPUT"
