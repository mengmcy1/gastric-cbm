...
脚本保存冻结清单快照、清单SHA、权重SHA和运行参数。已有同模型同模式输出时一律拒绝
覆盖；需要重跑时先人工核对并归档旧目录，再使用新的运行名。
...
全部K自动分析完成后，可生成保留九个K全部自动分析材料的医学生提交目录。完整原始
结果、K-Means模型和特征缓存仍留在服务器，不进入提交目录：

```bash
python 程序/MOCE/正式代码/build_k_sensitivity_submission.py
```
