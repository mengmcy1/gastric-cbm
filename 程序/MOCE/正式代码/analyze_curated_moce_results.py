...
DATA_DIR = os.path.join(
    PROJECT_DIR, "数据", "胃早癌概念提取训练集_裁剪后_v1_1"
)
MANIFEST_PATH = os.path.join(
    PROJECT_DIR, "结果", "MOCE聚类", "概念严格平衡_v1", "full",
    "efficientnet_b0", "frozen_manifest_snapshot.csv",
)
MOCE_DIR = os.path.join(
    PROJECT_DIR, "结果", "MOCE聚类", "概念严格平衡_v1", "full"
)
...
def source_name(patient_id):
    ...
    return patient_id.split("/", 1)[0] if "/" in patient_id else "未知来源"
