...
def quaternions_from_rotation_matrices(matrices: torch.Tensor) -> torch.Tensor:
    """Convert batch of rotation matrices to quaternions in wxyz order.

    Args:
        matrices: Rotation matrices with shape [..., 3, 3].

    Returns:
        Quaternions with shape [..., 4].

    Note:
        This operation is not differentiable and is performed on CPU.
    """
...
def get_cross_product_matrix(vectors: torch.Tensor) -> torch.Tensor:
    """Generate cross product matrices for 3D vectors.

    Args:
        vectors: Input vectors with shape [..., 3].

    Returns:
        Cross-product matrices with shape [..., 3, 3].
    """
