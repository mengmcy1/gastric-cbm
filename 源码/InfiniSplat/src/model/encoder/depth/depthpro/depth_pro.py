...
class DepthPro(nn.Module):
    ...
    def infer(
        ...
        # DepthPro upstream assumes scalar focal input. InfiniSplat passes per-sample focal
        # lengths for batched training, so reshape explicitly to broadcast over BCHW.
        if not torch.is_tensor(f_px):
            f_px = torch.tensor(
                f_px,
                device=canonical_inverse_depth.device,
                dtype=torch.float32,
            )
        else:
            f_px = f_px.to(device=canonical_inverse_depth.device, dtype=torch.float32)

        if f_px.ndim == 0:
            f_px = repeat(f_px, "-> b", b=canonical_inverse_depth.shape[0])
        elif f_px.ndim != 1:
            raise ValueError(
                f"Expected focal tensor with shape [B] or scalar, got {tuple(f_px.shape)}"
            )

        inverse_depth = canonical_inverse_depth * rearrange(W / f_px, "b -> b 1 1 1")
        f_px = rearrange(f_px, "b -> b")
        ...
        depth = 1.0 / torch.clamp(inverse_depth, min=5e-3)
