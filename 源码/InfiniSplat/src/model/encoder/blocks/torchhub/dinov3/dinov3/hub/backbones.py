...
def load_state_dict_from_path_or_url(
    ...
    *,
    ...
    if local_path.is_file():
        return torch.load(local_path.resolve(), map_location=map_location)
    return torch.hub.load_state_dict_from_url(
        convert_path_or_url_to_url(path_or_url),
    ...
    **kwargs,
    ...
    if pretrained:
        if type(weights) is Weights and weights not in {Weights.LVD1689M, Weights.SAT493M}:
            ...
            state_dict = load_state_dict_from_path_or_url(
                weights,
                map_location="cpu",
                check_hash=check_hash,
            )
    ...
    **kwargs,
    ...
    if pretrained:
        if type(weights) is Weights and weights not in {Weights.LVD1689M, Weights.SAT493M}:
            ...
            state_dict = load_state_dict_from_path_or_url(weights, map_location="cpu")
        model.load_state_dict(state_dict, strict=True)
    return model
    ...
    **kwargs,
    ...
    if weights == Weights.LVD1689M:
        ...
        if len(matches) == 1:
            ...
            if "pretrain_sat493m" in weights_name:
                untie_global_and_local_cls_norm = True
            elif "pretrain_lvd1689m" not in weights_name:
