...
SHARP 预测出 3DGS 后，渲染阶段只需要改变相机位置/朝向即可生成新视角。
本文件负责:
  - 根据场景深度和焦距估计合理的小幅移动范围
  - 生成 swipe / shake / rotate / rotate_forward 等相机轨迹
  - 根据眼睛位置计算目标相机的 intrinsics / extrinsics

它不参与神经网络推理，只服务于 `sharp render` 和 `sharp predict --render`。
